...
void testjig_done(unsigned char state);
// </editor-fold>
...
//        testjig_done(2);
    ...
    else if((get_30v_current() >= 30))
        print_error("12v-5v supply", "Curr Error");
    ...
    testjig_timer = T0_500ms;
    while((ADC_get_val(ADC_5v_test) < P5_5v_test_min) && (ADC_get_val(ADC_5v_test) > P5_5v_test_max)&&(ADC_get_val(ADC_12v_test) < P2_12v_test_min) && (ADC_get_val(ADC_12v_test) > P2_12v_test_max) && (get_30v_current() < 40) && testjig_timer);
    if((ADC_get_val(ADC_5v_test) < P5_5v_test_min))
    ...
    else if((get_30v_current() >= 40))
        print_error("16v-5v supply", "Curr Error");
    else if(testjig_timer == 0)
...
//test EEPROM read/write
    ...
    RX_data = RX_one();
    if(RX_data != 0x20)
        print_error("EEPROM size", " Error 1");
    TX_one(0x21);
    RX_data = RX_one();
    if(RX_data != 0x00)
        print_error("EEPROM size", " Error 2");
    ...
    //test RF - RSSI
    RF_out = 1;
    __delay_ms(150); //tested to 100ms
    ...
    if(RX_data == 1)//New key
        ...
        print_screen("Test RF", "Decode Old -Fail");
    else if(RX_data == 3)//New key
...
void testjig_done(unsigned char state)
{
    while(1)
    {
        ...
        {
            case 1://pass
                print_screen("Testjig Pass", " Remove board");
                debug_fast_tx(11111);
                buzz_one(100);
                __delay_ms(200);
                buzz_one(100);
